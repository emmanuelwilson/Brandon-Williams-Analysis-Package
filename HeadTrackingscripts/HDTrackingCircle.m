...
redThresh = 0.005;          % Must be a value between 1 and 0. The lower the number the more sensitve it is
greenThresh = 0.001;        % Must be a value between 1 and 0. The lower the number the more sensitve it is
            ...
            if frame ~=0 && ~isempty(targetR)&& ~isempty(targetG)                   %skip frame unless green and red can be found
                ...
                for objectR = 1:length(targetR)                                     %for every red object...
                    for objectG = 1:length(targetG)                                 %for every green object...
                        ...
                        if(abs(targetR(objectR).Centroid(1)-targetG(objectG).Centroid(1))<LEDxDist && abs(targetR(objectR).Centroid(2)-targetG(objectG).Centroid(2))<LEDyDist && Mask(round(targetR(objectR).Centroid(2)),round(targetR(objectR).Centroid(1)))==1)
...
for i = 1 : length(HEADco)                                          %For the whole data set
    if (~isnan(HEADco(i,1)) && Er == 0) || (~isnan(HEADco(i,1)) && (Er == 1 && nany > 5) || (~isnan(HEADco(i,1)) && (Er == 1 && (i-1)<=nany)))   %if HEADco is not NaN and not an error or if there were more than 5 errors in a row then ... 
