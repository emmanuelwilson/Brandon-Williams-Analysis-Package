...
redThresh = 0.0001;          % Must be a value between 1 and 0. The lower the number the more sensitve it is
greenThresh = 0.0001;        % Must be a value between 1 and 0. The lower the number the more sensitve it is
ExtentLim = 0.2;            %Extent limit : ratio of bounding box area to full area
EccLim = 0.98;               %Eccentricity limit: roundness factor, 1=line, 0=circle
...
for i = 1 : length(HEADco)                                          %For the whole data set
    if (~isnan(HEADco(i,1)) && Er == 0) || (~isnan(HEADco(i,1)) && (Er == 1 && nany > 5)) || (i == 2 && Er == 1 && nany == 1)   %if HEADco is not NaN and not an error or if there were more than 5 errors in a row then ... 
