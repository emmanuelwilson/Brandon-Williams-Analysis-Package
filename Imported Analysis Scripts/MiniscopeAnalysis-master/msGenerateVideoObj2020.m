function ms = msGenerateVideoObj2020(dirName, filePrefix)
...
%   Detailed explanation goes here
    ...
    for i=1:length(aviFiles)
        ...
        if (isnumeric(str2num(aviFiles(i).name(1:end-4))))
...
%         [folder filesep num2str(filePrefix) num2str(i) '.avi']
        ms.vidObj{i} = VideoReader([dirName filesep num2str(filePrefix) num2str(i-1) '.avi']);
    ...
    for i=1:length(csvFiles)
        if strcmp(csvFiles(i).name,'timestamp.cvs')
            fileID = fopen([dirName filesep csvFiles(i).name],'r');
...
%                 (sum(camNum==j) == ms.numFrames)
        ...
        if strcmp(csvFiles(i).name, 'settings_and_notes.dat')
            fileID = fopen([dirName filesep csvFiles(i).name],'r');
