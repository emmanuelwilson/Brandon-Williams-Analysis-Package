...
function msRun2020_newSoft_NoiseReprocessTEST(p)
...
while 1    
    ...
    ms = msNormCorre2020_exceptions(ms,isnonrigid,badframes);
    ...
    meanCaldiff = diff(mean(ms.FiltTraces,2));
    stdmeanCaldiff = std(meanCaldiff);
    badframes = find(meanCaldiff > mean(meanCaldiff) + stdmeanCaldiff*2);
    badframes(badframes < 100) = [];
    if isempty(badframes) || redo
        break
    else
        redo = true;
    end        
