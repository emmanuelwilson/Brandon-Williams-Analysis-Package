function ms = msNormCorre2020_exceptions(ms,isnonrigid,badframes)
...
blankframe = zeros(ms.height, ms.width,'uint8');
framecount = 0;
for video_i = 1:ms.numFiles;
    ...
    Yf = read_file2020(name);    
    framecount = framecount + length(Yf(1,1,:));
    if ~isempty(find(badframes <= framecount))
        badf = badframes(badframes <= framecount) - framecount + length(Yf(1,1,:));
        for i = 1 : length(badf)
            Yf(:,:,badf(i)) = blankframe;
        end
        badframes = badframes(badframes > framecount);
    end
    Yf = single(Yf);
