...
%             thresh = -inf;
    ...
    figure(1)
    set(gcf,'position',[50 50 900 125.*length(dat)])
    for mi = 1:length(dat)
    ...
    for mi = 1:length(dat)
        ...
        for di = 1:2:length(dat(mi).maps.rooms(1,:))
            ...
            gT = [dat(mi).trace{di}(dat(mi).isAligned{di},:) ...
                dat(mi).trace{di+1}(dat(mi).isAligned{di+1},:)];
            gT = gT(dat(mi).rely{di},:);
            doP = [dat(mi).path{di} dat(mi).path{di+1}];
            m = mkTraceMaps([dat(mi).path{di} dat(mi).path{di+1}],gT,[]);
            ...
            for part = 0:floor(length(gT(:,1))/prod(doK))
                ...
                com = [dat(mi).mostrecent.common{di} dat(mi).mostrecent.common{di+1}];
                uni = [dat(mi).mostrecent.unique{di} dat(mi).mostrecent.unique{di+1}];
                isInRoom = [dat(mi).isinroom{di} dat(mi).isinroom{di+1}];
...
%                     plot(doP(2,1:step:end),doP(1,1:step:end), ...
%                     'color',[0.6 0.6 0.6],'linewidth',1) %[0.8 0.8 0.8]
%                     hold on
% 
%                     plot(doP(2,logical(gT(part.*prod(doK)+k,:))),...
%                         doP(1,logical(gT(part.*prod(doK)+k,:))),...
...
%                     hold on
% 
%                     plot(doP(2,logical(gT(part.*prod(doK)+k,:))&com),...
%                     doP(1,logical(gT(part.*prod(doK)+k,:))&com),...
%                     'color',[0.5 0.1 1],'linestyle','none','marker','o',...
%                     'markerfacecolor',[0.5 0.1 1],'markersize',1);
% 
%                     plot(doP(2,logical(gT(part.*prod(doK)+k,:))&uni),...
%                         doP(1,logical(gT(part.*prod(doK)+k,:))&uni),...
%                         'color',[0.1 1 1],'linestyle','none','marker','o',...
%                         'markerfacecolor',[0.1 1 1],'markersize',1);
%                     image(bsxfun(@times,m(:,:,part.*prod(doK)+k),cstack))
                ...
                outP = ['Plots/TracePlots_Combined_Differentiated/ThreeRooms/Animal_' num2str(mi)  '/Day_' num2str(di) '_Partition_' num2str(part+1)];
                saveFig(gcf,outP,'tiff')
                saveFig(gcf,outP,'pdf')
                close all
                drawnow
...
%         simXPixel{1} = cat(3,simXPixel{1},nanmean(cat(3,simXLocation{1,2},simXLocation{2,1}),3));
%         simXPixel{2} = cat(3,simXPixel{2},nanmean(cat(3,simXLocation{2,3},simXLocation{3,2}),3));
%         simXPixel{3} = cat(3,simXPixel{3},nanmean(cat(3,simXLocation{1,3},simXLocation{3,1}),3));
