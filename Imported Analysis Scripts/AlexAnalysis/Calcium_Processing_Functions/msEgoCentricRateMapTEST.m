function [out, detailed]= msEgoCentricRateMapTEST(ms,HD,tracking, frameMap, thresh, pixX, pixY, varargin)
...
mkdir EBCresultsCNMFE0.004                                    %Create new folder within current directory
direct = dir ('EBCresultsCNMFE0.004');                         %Access new folder
...
for cellNum = 1 : length(ms.firing(1,:))    
    firing = ms.firing_cnmfe(:,cellNum);                          %Extract firing trace
    ...
    if(~isempty(ifire))
        ...
        if any(nspk(:)>0)
            ...
            %{
            %% Begin what was in EBCTest
%             convert to spike train
%             t = ms.timestamp;
%             [spk] = histc(ms.cell_time, t);
            % Fit each angle
%             xf = 0:0.1:max(distanceBins);
%             fv = nan(length(thetaBins),length(xf));
%             for i = 1:size(dis,2)                              
%                 % transform to weibel distribution of distance
%                 y=rm(:,i); y = y-min(y); y= y/max(y);
%                 x=distanceBins(2:end); 
%                 x = x(:); y = y(:);                
%                 ft = fittype( 'weibull' );
...
%                     pause('query')
%                 else
%                     [fitresult, gof(i)] = fit(x, y, ft, opts );                
%                 
%                     pred = fitresult(dis(:,i));
%                     [betaAll(i,:), devAll(i), statsAll(i)] = glmfit(pred, firing, 'poisson');
%                     fv(i,:) = fitresult(xf);
%                 end
%             end
%             [~, thetaPeak] = min(devAll);
%             dev = devAll(thetaPeak);
%             
%             % transform to weibel distribution of distance
%             y=rm(:,thetaPeak); y = y-min(y); y= y/sum(y);
...
%             [~,PrefDistance] = max(WeibullFit);PrefDistance=WeibullFitX(PrefDistance);
%             
%             %% Package the output
%             out.rm_prefAngle = fr;
%             detailed.stats = statsAll;
%             out.ebcPred = ebcPred;
%             
%             out.PrefDis = PrefDistance;
%     
%             out.DevianceWeibull = dev;
%             out.WeibullEstimates = [fitresult.a fitresult.b];
%             out.WeibullFit = WeibullFit;
%             out.WeibullFitX = WeibullFitX;
% %             
% %     
%     out.PrefTheta = thetaPeak;
%     
%     out.distanceBins = distanceBins;
%     
%     out.beta = betaAll(thetaPeak,:);
% 
...
%             %EBC Metric
            ...
            %Save Results
            saveas(gcf,['EBCresultsCNMFE0.004/',num2str(cellNum),'EBC.jpg']); %saving figure as a picture file (.jpg) in the new folder "EBCresults"
            ms.ind_fire = NaN(ms.numFrames,length(ms.firing(1,:))); %Indices of neuron activity/firing
            ms.cell_x = NaN(ms.numFrames,length(ms.firing(1,:)));   %X coordinate of locations where cell fired
            ms.cell_y = NaN(ms.numFrames,length(ms.firing(1,:)));   %Y cooridnates of locations where cell fired
            ms.cell_time = NaN(ms.numFrames,length(ms.firing(1,:)));%Time at when cell fired
