function [out, detailed]= msEgoCentricRateMapProb(ms,HD,tracking, frameMap, pixX, pixY, varargin)
...
ratemaps = zeros(37,361,length(ms.trace(1,:)));        %Probability ratemap values
...
probMap = zeros(80,80);                                 %Probablility open feild rate map
probind = NaN(length(probMap)*length(probMap),1000);    %Probability ratemap index
...
freqFire = zeros(1,length(ms.trace(1,:)));             %Firing frequency for each cell
...
mkdir EBCresultsProbPOLARtestGONonZero2                              %Create new folder within current directory
direct = dir ('EBCresultsProbPOLARtestGONonZero2');                  %Access new folder
mrtot = 0;                                              %total MRL
counter = 0 ;                                           %counter
...
% ms.trace = ms.trace+abs(min(ms.trace));
...
for cellNum = 1 : length(ms.trace(1,:))
    ...
    if frameMap(length(frameMap)) < length(ms.trace)
        ...
        ms.firing_cnmfe(:,cellNum) = ms.firing_cnmfe(frameMap,cellNum); 
    end
    firing = ms.trace(:,cellNum);%ms.firing(:,cellNum);                          %Extract firing trace
    ...
    if(~isempty(ifire))
        for j = 1 : length(ifire)
            ...
            if ms.cell_y(j,cellNum) > 0
                n=1;
                while ~isnan(probind(sub2ind(size(probMap),round(pixY*ms.cell_y(j,cellNum)),round(pixX*ms.cell_x(j,cellNum))),n))
                    n = n+1;
                end
                probind(sub2ind(size(probMap),round(pixY*ms.cell_y(j,cellNum)),round(pixX*ms.cell_x(j,cellNum))),n) = firing(ifire(j));
            end
        end     
...
%             colorbar
            ...
            %Save Results
            saveas(gcf,['EBCresultsProbPOLARtestGONonZero2/',num2str(cellNum),'EBC.jpg']); %saving figure as a picture file (.jpg) in the new folder "EBCresults"
            ms.ind_fire = NaN(ms.numFrames,length(ms.trace(1,:))); %Indices of neuron activity/firing
            ms.cell_x = NaN(ms.numFrames,length(ms.trace(1,:)));   %X coordinate of locations where cell fired
            ms.cell_y = NaN(ms.numFrames,length(ms.trace(1,:)));   %Y cooridnates of locations where cell fired
            ms.cell_time = NaN(ms.numFrames,length(ms.trace(1,:)));%Time at when cell fired
