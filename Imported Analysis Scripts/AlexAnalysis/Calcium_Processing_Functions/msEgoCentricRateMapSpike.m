function [out, detailed]= msEgoCentricRateMapSpike(ms,HD,tracking, frameMap, spikes, pixX, pixY, varargin)
...
ms.ind_fire = NaN(ms.numFrames,length(ms.trace(1,:))); %Indices of neuron activity/firing
ms.cell_x = NaN(ms.numFrames,length(ms.trace(1,:)));   %X coordinate of locations where cell fired
ms.cell_y = NaN(ms.numFrames,length(ms.trace(1,:)));   %Y cooridnates of locations where cell fired
ms.cell_time = NaN(ms.numFrames,length(ms.trace(1,:)));%Time at when cell fired
...
mkdir EBCresultsMLSpikeFULLnot                                    %Create new folder within current directory
direct = dir ('EBCresultsMLSpikeFULLnot');                         %Access new folder
degBins = (-90:2:269);                                  %Angle bins for EBC metric polar plot
degBins = degBins';
degBins = deg2rad(degBins);
mrtot = 0;
counter = 0 ;
freqFire = zeros(1,length(ms.trace(1,:)));
mrall = freqFire;
freqMax = freqFire;
ratemaps = zeros(37,361,length(ms.trace(1,:)));
spike = spikes.spikest./0.0333;
...
for cellNum = 1 : length(ms.trace(1,:))
    if frameMap(length(frameMap)) < length(ms.trace)
        ms.trace(:,cellNum) = ms.trace(frameMap,cellNum);   %Sink trace if not already sinked 
    end
    if(~isempty(spike(:,cellNum)))
        for j = 1 : length(find(spike(:,cellNum)))            
            spike(j,cellNum) = round(spike(j,cellNum));
            ms.ind_fire(j,cellNum) = spike(j,cellNum);                      %Add firing index to ms struct
            ms.cell_x(j,cellNum) = tracking((frameMap(spike(j,cellNum))));  %X postion of mouse during firing at sinked time
            ms.cell_y(j,cellNum) = tracking((frameMap(spike(j,cellNum))),2);%Y position of mouse during firing at sinked time
            ms.cell_time(j,cellNum) = ms.timestamp(spike(j,cellNum));       %Physiological time of firing
            ms.HDfiring(j,cellNum) = HD(frameMap(spike(j,cellNum)));        %Head Direction of mouse at time of neural firing
        end          
...
%             colorbar
            ...
            %Save Results
           saveas(gcf,['EBCresultsMLSpikeFULLnot/',num2str(cellNum),'EBC.jpg']); %saving figure as a picture file (.jpg) in the new folder "EBCresults"
            ms.ind_fire = NaN(ms.numFrames,length(ms.trace(1,:))); %Indices of neuron activity/firing
            ms.cell_x = NaN(ms.numFrames,length(ms.trace(1,:)));   %X coordinate of locations where cell fired
            ms.cell_y = NaN(ms.numFrames,length(ms.trace(1,:)));   %Y cooridnates of locations where cell fired
            ms.cell_time = NaN(ms.numFrames,length(ms.trace(1,:)));%Time at when cell fired
            ...
            ratemaps(:,:,cellNum) = rm;
            mrtot = mrtot + mr;
            freqFire(1,cellNum) = length(find(spike(:,cellNum)));
            mrall(1,cellNum) = mr;
            clf
        end
    end
...
xlabel('frequency (Hz)')
savefig('ThreshSpikeFixFreqHist.fig')
figure
histogram(mrall)
title('MRL distribution at 0.1 treshold')
ylabel('Number of Cells')
xlabel('Mean Resultant Length')
savefig('ThreshSpikeFix_MRL_Hist.fig')
end
