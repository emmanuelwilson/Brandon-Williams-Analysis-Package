...
for startFrameNum=frameLimits(1):stepSize:min([frameLimits(2) vidObj.numFrames-stepSize]) %steps through data video
    ...
    %-------------------------- Zaki --------------------------------------
%     im1 = imadjust(im2double(read(videoObject, startFrameNum)), [0.2 0.6], [0 1]);
%         im2 = filter2(hLarge,im2double(im1));
%         im3 = im2double(im1)-im2double(im2);
%         im4 = imadjust(im3, [0 0.4], [0 1]);
%         %im5 = imerode(im4, se2);
%         savedVid(:,:,(floor(startFrameNum/1)+1)) = imerode(im4, se2);
    %----------------------------------------------------------------------
    ...
    for propNum = 1:length(bwProps)
        ...
        if (bwProps(propNum).Area < 1000 && bwProps(propNum).Area >=20)  % was 500 and 20
    ...
    %----------------------------------------------------------------------
    %vidObj.savedVid(:,:,startFrameNum) = frameMax-frameBase; %#################################
    %Plotting (Can be commented out)
    if plotting == true
        figure(1);
        clf
        hold off
%     imshow(frameMax-frameBase);  %###########################################
