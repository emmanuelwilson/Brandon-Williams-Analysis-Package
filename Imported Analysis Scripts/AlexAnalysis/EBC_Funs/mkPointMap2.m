function [m prepStack] = mkPointMap2(P,HD,T,mask,walls,prepStack)
...
%     tsmap = nansum(smap,4);
%     tsmap(tsmap<300) = nan;
%     tsmap = imfilter(repmat(tsmap,[3 1 1 1]),fspecial('gauss',round([5.*kern 5.*kern]),kern),'same','replicate');
%     tsmap = tsmap([length(tsmap(:,1,1,1)).*(1./3)]+1:[length(tsmap(:,1,1,1)).*(2./3)],:,:,:);
%     
%     m = maps;
%     bad = isnan(m);
%     m(bad) = 0;
%     m = imfilter(repmat(m,[3 1 1 1]),fspecial('gauss',round([5.*kern 5.*kern]),kern),'same','replicate');
%     m = m([length(m(:,1,1,1)).*(1./3)]+1:[length(m(:,1,1,1)).*(2./3)],:,:,:);
%     
