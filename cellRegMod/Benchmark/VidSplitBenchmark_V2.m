...
function [] = VidSplitBenchmark_V2(folderpath)
format long
...
imsecmask = zeros(length(refFrame(:,1)),length(refFrame(1,:)),length(x));
for i = 1 : length(x)
    xmin(i) = x(i) - round(dims(1)/2);
    ymin(i) = y(i) - round(dims(2)/2);
    imsecmask(xmin(i):xmin(i)+dims(2),ymin(i):ymin(i)+dims(1),i) = 1;
end
...
for i = 1 : length(x)
    ...
    if  spill && i == length(x)
        ...
        for j = 1 : sL-over
            writeVideo(v,bigvid(int32(j+(sL*(i-1)))).cdata.*uint8(imsecmask(:,:,i)));
        end
    else
        for j = 1 : sL
            writeVideo(v,bigvid(int32(j+(sL*(i-1)))).cdata.*uint8(imsecmask(:,:,i)));
