...
function [map] = registerCellsNoGUI(folderpath,subfigfold, s1, s2)
% Defining the results_directory and creating the figures_directory:
results_directory=[folderpath, '\Results', '\', subfigfold];
...
for i = 1:length(folder)    
    if ~folder(i).isdir
        file_names{count}= [folder(i).folder,'/', folder(i).name];
        count = count +1;
    end
end
...
for i = 1 : length(file_names)
    if contains(file_names{i}, ['ms',num2str(s1),'.mat']) && (isempty(newnames{1}))
        newnames{1} = file_names{i};
    elseif endsWith(file_names{i}, [num2str(s2),'.mat'])
        newnames{2} = file_names{i};
    end
end
file_names = newnames; 
