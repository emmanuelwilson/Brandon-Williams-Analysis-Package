...
for i = 3 : length(folder)                                                 %Look through folder items
    if folder(i).isdir == 1                                                %Will proceed only if folder item is another folder/directory
        ...
        if ~isempty(find(strcmp(fnames,'msDeconvolvedNEW.mat'),1)) && ~isempty(find(strncmp(fnames,'frameMap',1),1)) && ~isempty(find(strncmp(fnames,'HeadTrackingData',1),1)) && isempty(find(strncmp(fnames,'EBCevenOddSplitParallelDeconvolved_CircSHUFFLEDonut3_100itt_redo',62),1))
            if ~isempty(find(strncmp(fnames,'EBCresultsEvenOddSplitParallel_Deconvolved_Donut3FIX_redo',1),1))                
                cd([pwd,'/',folder(i).name]);                                  %Change current folder
                subdir2 = [pwd,'\EBCresultsEvenOddSplitParallel_Deconvolved_Donut3FIX_redo'];                                 %file path
                ...
                if ~isempty(find(strcmp(fnames2,'EBCstats.mat'),1)) %&& isempty(find(strcmp(fnames2,'EBCevenOddSplitParallelDeconvolved_CircSHUFFLE.mat'),1))
                    load('EBCresultsEvenOddSplitParallel_Deconvolved_Donut3FIX_redo\EBCstats.mat')
                    load('msDeconvolvedNEW.mat')
                    load('frameMap.mat')
