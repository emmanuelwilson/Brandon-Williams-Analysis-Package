function [out]= EBCShufflingContinuous(ms,HD,tracking, frameMap, pixX, binarize,elim)
...
MRLsave = zeros(length(firing(1,:))*100,1);                                 %saves all values for all MRL values
...
for itteration = 1 : 100                                                    %Loop the number of times you wish to shuffle
    ...
    for cellNum = length(firing(1,:))/3+1 : (length(firing(1,:))/3)*2
