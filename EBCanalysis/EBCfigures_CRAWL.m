%% Crawl script for EBC figure generation
...
for i = 3 : length(folder)                                                 %Look through folder items
    if folder(i).isdir == 1                                                %Will proceed only if folder item is another folder/directory
        ...
        if ~isempty(find(strcmp(fnames,'msDeconvolved.mat'),1)) && ~isempty(find(strncmp(fnames,'frameMap',1),1)) && ~isempty(find(strncmp(fnames,'HeadTrackingData',1),1)) && isempty(find(strncmp(fnames,'EBCresultsEvenOddSplitParallelNEW_Deconvolved_Donut3FIX',59),1))
            if ~isempty(find(strncmp(fnames,'EBCresultsEvenOddSplitParallelNEW_Deconvolved_Donut3',1),1))                
                ...
                if ~isempty(find(strcmp(fnames2,'EBCstats.mat'),1)) %&& isempty(find(strcmp(fnames2,'EBCevenOddSplitParallelDeconvolved_CircSHUFFLE.mat'),1))
                    ...
                    msEgoCentricRateMapSplitEvenOddParallel_V3(ms,HDdeg,SINKdata, frameMap, out.dimX, out.dimY, 1, out.QP)                                                      %Run analysis
                end
                cd(oldCD);            
            elseif ~isempty(find(strncmp(fnames,'EBCresultsEvenOddSplitParallelNEW_deconvolvedFIX',1),1))
                ...
                if ~isempty(find(strcmp(fnames2,'EBCstats.mat'),1)) %&& isempty(find(strcmp(fnames2,'EBCevenOddSplitParallelDeconvolved_CircSHUFFLE.mat'),1))
                    ...
                    if length(fieldnames(out)) == 16
                        ...
                        load('frameMap.mat')
                        load('HeadTrackingData.mat')
                        msEgoCentricRateMapSplitEvenOddParallel_V3(ms,HDdeg,SINKdata, frameMap, out.dimX, out.dimX, 1, out.QP)                                                      %Run analysis
                    end
                end
                cd(oldCD);
