function [out]= msEgoCentricRateMapSplitEvenOddParallelSweetMilkParallel(ms,HD,tracking, frameMap, dimX, dimY, deconvolve, QPO, QPW)
...
name = 'EBCevenOddSplitParallelDeconvolved_Donut3SWEETMILK.mat';
...
ratemaps1O = zeros(FOVsize,120,length(ms.FiltTraces(1,:)));                 %Probability ratemap values
ratemaps2O = zeros(FOVsize,120,length(ms.FiltTraces(1,:)));                 %Probability ratemap values

ratemapsW = zeros(FOVsize,120,length(ms.FiltTraces(1,:)));                  %Probability ratemap values
ratemaps1W = zeros(FOVsize,120,length(ms.FiltTraces(1,:)));                 %Probability ratemap values
ratemaps2W = zeros(FOVsize,120,length(ms.FiltTraces(1,:)));                 %Probability ratemap values
...
minDist = [1:3];                                                            %minimum distance ignored
...
parfor cellNum = 1 : length(ms.FiltTraces(1,:))
    ...
    if(~isempty(ifire) && ~isempty(ifire1) && ~isempty(ifire2))
        ...
        thetaBins3d = deg2rad(linspace(-180,180,round(size(disW,2)/3)));    %angle bins 3 degrees
        occO = NaN(length(thetaBins), length(distanceBins));                %Object total occupancy bins
...
out.ratemapO = ratemapsW;
out.ratemap1O = ratemaps1W;
out.ratemap2O = ratemaps2W;
out.dimX = dimX;
out.dimY = dimY;
out.QPO = QPO;
out.QPW = QPW;
