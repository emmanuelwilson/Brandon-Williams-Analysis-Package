function [out, detailed]= msEgoCentricRateMapSplitEvenOdd(ms,HD,tracking, frameMap, pixX, pixY, binarize, varargin)
...
ratemaps1 = zeros(FOVsize,361,length(ms.FiltTraces(1,:)));        %Probability ratemap values
ratemaps2 = zeros(FOVsize,361,length(ms.FiltTraces(1,:)));        %Probability ratemap values
...
ind_fire2 = NaN(ms.numFrames,1);
cell_x = NaN(ms.numFrames,1);   %X coordinate of locations where cell fired
cell_y = NaN(ms.numFrames,1);   %Y cooridnates of locations where cell fired
cell_time = NaN(ms.numFrames,1);%Time at when cell fired
...
counter1 = 0;
counter2 = 0;
correlation = zeros(length(ms.FiltTraces(1,:)),1);
fps = 30;                                               %Frames per second
spf = 1/fps;                                            %Seconds per frame
ms.timestamp = frameMap.*spf;                           %time stamp in seconds
county = 0;
timevar = zeros(length(ms.FiltTraces(1,:)),1);
    ...
    for i = 1 : mins
        if mod(i,2) == 0
            ...
            elseif i < mins
                ...
                ifire1(end+1:end+1800)= ((1800*i)-1799:1800*i);
                dis1(end+1:end+1800,:) = dis((1800*i)-1799:1800*i,:);
            else
                fire1(end+1:end+(length(fire)-(mins-1)*1800)) = fire(((1800*i)-1799:end),1);
                ifire1(end+1:end+(length(fire)-(mins-1)*1800)) = ((1800*i)-1799:length(fire));
...
%     ifire1 = ifire1(find(fire1));
%     ifire2 = ifire2(find(fire2));
    if(~isempty(ifire) && ~isempty(ifire1) && ~isempty(ifire2))
        for j = 1 : length(ifire)
            ...
            cell_time(j) = ms.timestamp(ifire(j));       %Physiological time of firing
            ...
            if j < length(ifire1)
                ...
                cell_x1(j) = tracking((frameMap(ifire1(j))));  %X postion of mouse during firing at sinked time
                cell_y1(j) = tracking((frameMap(ifire1(j))),2);%Y position of mouse during firing at sinked time
                cell_time1(j) = ms.timestamp(ifire1(j));       %Physiological time of firing
                HDfiring1(j) = HD(frameMap(ifire1(j)));        %Head Direction of mouse at time of neural firing
            end
            %second half
            if j < length(ifire2)
                ind_fire2(j) = ifire2(j);                      %Add firing index to ms struct
                cell_x2(j) = tracking((frameMap(ifire2(j))));  %X postion of mouse during firing at sinked time
                cell_y2(j) = tracking((frameMap(ifire2(j))),2);%Y position of mouse during firing at sinked time
                cell_time2(j) = ms.timestamp(ifire2(j));       %Physiological time of firing
                HDfiring2(j) = HD(frameMap(ifire2(j)));        %Head Direction of mouse at time of neural firing
            end
        end     
...
%         distanceBins = distanceBins;
        ...
        for i = 1:length(thetaBins)
            ...
            for k = 1:length(distanceBins)-1
                ...
                %first half
                inds1 = t1>=distanceBins(k) & t1<distanceBins(k+1);                %filter through the boundary distances
                ...
                %second half
                inds2 = t2>=distanceBins(k) & t2<distanceBins(k+1);                %filter through the boundary distances
...
%             title('nspk 1st half')
            ...
            subplot(n,m,c);c=c+1;
            % the +pi/2 brings "forwards" to "up"
            [t2, r2] = meshgrid(wrapTo2Pi(thetaBins1+pi/2), distanceBins(1:end-1));
            [x, y] = pol2cart(t2,r2);
...
%             set(gca, 'YDir','Normal','CLim',[0 prctile(rm(:), 99)])
            ...
            info1 = ['Max: ' num2str(maxrm1) ' Correlation: ' num2str(corr)];
...
%             degBins2 = rad2deg(degBins2);
            ...
            ind_fire = NaN(ms.numFrames,1); %Indices of neuron activity/firing
            cell_x = NaN(ms.numFrames,1);   %X coordinate of locations where cell fired
            cell_y = NaN(ms.numFrames,1);   %Y cooridnates of locations where cell fired
            cell_time = NaN(ms.numFrames,1);%Time at when cell fired
            HDfiring = ms.ind_fire;                              %Indices of neuron activity for head direction 
