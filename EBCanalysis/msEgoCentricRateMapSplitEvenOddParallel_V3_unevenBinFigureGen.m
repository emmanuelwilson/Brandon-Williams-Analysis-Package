function [out, detailed]= msEgoCentricRateMapSplitEvenOddParallel_V3_unevenBinFigureGen(ms,HD,tracking, frameMap, dimX, dimY, deconvolve, QP,cellIndex,DistBinSize,DegBinSize,allocentric)
...
name = ['EBCresultsEvenOddSplitParallel_Deconvolved_Donut3_Bin_D',num2str(round(DistBinSize)),'A',num2str(DegBinSize),'FigureGen.mat'];
...
ratemaps = zeros(length(distanceBins)-1,360/DegBinSize,length(ms.FiltTraces(1,:)));        %Probability ratemap values
...
%             figure('visible','off');
            ...
            %Allocentric Heatmap
            subplot(n,m,c);c=c+2;
            allo = rot90(fliplr(allocentric(:,:,cellNum)));
            b = imagesc(allo);
            colormap(jet)
            set(b,'AlphaData',~isnan(allo))
            axis off
            axis square
            freezeColors
...
%             set(gca, 'YDir','Normal','CLim',[0 prctile(rm(:), 99)])
            ...
            if deconvolve == 1
                ...
                axis off
                axis square
                scatter(cx,-cy,30,HDfiring(:),'filled')
                set(gca,'YDir','Normal')
                caxis([0 360])
                title('Traj')                
            else
...
%             county  = county+1;
            ...
            %Save Results
            saveas(gcf,[name,'/',num2str(cellNum),'EBC.fig']); %saving figure as a picture file (.jpg) in the new folder "EBCresults"            
            saveas(gcf,[name,'/',num2str(cellNum),'EBC.eps']);
