function [out, detailed]= msEgoCentricRateMapSplitEvenOddParallel_V3_unevenBINs(ms,HD,tracking, frameMap, dimX, dimY, deconvolve, QP,cellIndex,DistBinSize,DegBinSize)
...
% DistBinSize = 1.2;
% DegBinSize = 6;
ratemaps = zeros(round(FOVsize/DistBinSize)-1,360/DegBinSize,length(ms.FiltTraces(1,:)));        %Probability ratemap values
ratemaps1 = zeros(round(FOVsize/DistBinSize)-1,360/DegBinSize,length(ms.FiltTraces(1,:)));        %Probability ratemap values
ratemaps2 = zeros(round(FOVsize/DistBinSize)-1,360/DegBinSize,length(ms.FiltTraces(1,:)));        %Probability ratemap values
...
if length(distanceBins) < length(ratemaps(:,1,1))+1    
    distanceBins(length(ratemaps(:,1,1))) = round(length(ratemaps(:,1,1))*DistBinSize);
elseif length(distanceBins) > length(ratemaps(:,1,1))+1
    differ = length(distanceBins)-(length(ratemaps(:,1,1))+1);
    distanceBins(end-differ:end) = [];
    distanceBins(length(ratemaps(:,1,1))+1) = round(distanceBins(length(ratemaps(:,1,1)))+1);
end
