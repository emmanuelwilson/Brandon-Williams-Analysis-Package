...
function [ratios] = AreaRatio_HighvsRest(corrangles,window)
maxval = [];
minval = [];
    ...
    if any(any(~cellfun(@isempty,corrangles(i,:,:))))
        ...
        for j = 1 : length(y)
            ...
            if maxpeak < window+1
                ...
                maxval = sum(corrrots{y(j),x(j)}([maxpeak-botadd:maxpeak+window, end - topadd : end]))-(window*minamp);
            ...
            elseif maxpeak > 60 - window
                ...
                maxval = sum(corrrots{y(j),x(j)}([maxpeak-window:maxpeak+topadd, 1 : botadd]))-(window*minamp);
                mincorr = corrrots{y(j),x(j)};
                mincorr([maxpeak-window:maxpeak+topadd, 1 : botadd]) = [];
                minval = sum(mincorr)-(length(mincorr)*minamp);
            else
                maxval = sum(corrrots{y(j),x(j)}(maxpeak-window:maxpeak+window))-(window*minamp);
                mincorr = corrrots{y(j),x(j)};
