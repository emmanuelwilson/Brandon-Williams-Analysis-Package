function [out, detailed]= msEgoCentricRateMapSweetMilk(ms,HD,tracking, frameMap, pixX, pixY, binarize, varargin)
...
mkdir EBCresultsBinarizeObject                          %Create new folder within current directory
mrtot = 0;                                              %total MRL
counter = 0 ;                                           %counter
...
%             colorbar
            ...
            %Save Results
            saveas(gcf,['EBCresultsBinarizeObject/',num2str(cellNum),'EBC.jpg']); %saving figure as a picture file (.jpg) in the new folder "EBCresults"
            ms.ind_fire = NaN(ms.numFrames,length(ms.FiltTraces(1,:))); %Indices of neuron activity/firing
            ms.cell_x = NaN(ms.numFrames,length(ms.FiltTraces(1,:)));   %X coordinate of locations where cell fired
            ms.cell_y = NaN(ms.numFrames,length(ms.FiltTraces(1,:)));   %Y cooridnates of locations where cell fired
            ms.cell_time = NaN(ms.numFrames,length(ms.FiltTraces(1,:)));%Time at when cell fired
