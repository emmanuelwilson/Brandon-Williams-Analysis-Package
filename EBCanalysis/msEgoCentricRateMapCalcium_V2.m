function [out, detailed]= msEgoCentricRateMapCalcium_V2(ms,HD,tracking, frameMap, pixX, pixY, binarize, varargin)
...
ratemaps = zeros(30,120,length(ms.FiltTraces(1,:)));        %Probability ratemap values
ratemaps1 = zeros(30,120,length(ms.FiltTraces(1,:)));        %Probability ratemap values
ratemaps2 = zeros(30,120,length(ms.FiltTraces(1,:)));        %Probability ratemap values
...
degBins1 = (-180:3:179);                                  %Angle bins for EBC metric polar plot
degBins1 = degBins1';                                     %reorient bins
% degBins1 = rad2deg(degBins1);                             %Convert to radians
degBins2 = (-180:3:179);                                  %Angle bins for EBC metric polar plot
...
name = 'EBCresultsCNMFE_Split_Binarized_Driftless_3Deg'; 
...
for cellNum = 1 : length(ms.FiltTraces(1,:))
    ...
    if(~isempty(ifire) && ~isempty(ifire1) && ~isempty(ifire2))
        ...
        for i = 1 : round(length(thetaBins)/3)
            occ3d(i,:) = sum(occ(i*3-2:i*3,:),1);
            occ13d(i,:) = sum(occ1(i*3-2:i*3,:),1);
            occ23d(i,:) = sum(occ2(i*3-2:i*3,:),1);
            
            nspk3d(i,:) = sum(nspk(i*3-2:i*3,:),1);
            nspk13d(i,:) = sum(nspk1(i*3-2:i*3,:),1);
            nspk23d(i,:) = sum(nspk2(i*3-2:i*3,:),1);            
        end
...
%             title('nspk')
            ...
            subplot(n,m,c);c=c+1;
            % the +pi/2 brings "forwards" to "up"
            [t2, r2] = meshgrid(wrapTo2Pi(thetaBins3d+pi/2), distanceBins(1:end-1));
            [x, y] = pol2cart(t2,r2);
...
%             end
            ...
            xs = metric.*cos(degBins'); % average
            ys = metric.*sin(degBins');
            
            coordlims=axis;
            
            ang_hd = atan2(mean(ys),mean(xs)); % mean direction
            
            mr = (cos(ang_hd).*sum(xs) + sin(ang_hd).*sum(ys)) / sum(metric); % mean resultant length
...
%             title('nspk 1st half')
            ...
            subplot(n,m,c);c=c+1;
            % the +pi/2 brings "forwards" to "up"
            [t2, r2] = meshgrid(wrapTo2Pi(thetaBins3d+pi/2), distanceBins1(1:end-1));
            [x, y] = pol2cart(t2,r2);
...
%             end
            ...
            xs = metric1.*cos(degBins'); % average
            ys = metric1.*sin(degBins');
            
            coordlims=axis;
            
            ang_hd1 = atan2(mean(ys),mean(xs)); % mean direction
            
            mr1 = (cos(ang_hd1).*sum(xs) + sin(ang_hd1).*sum(ys)) / sum(metric1); % mean resultant length
...
%             title('nspk 2nd half')
            ...
            subplot(n,m,c);c=c+1;
            % the +pi/2 brings "forwards" to "up"
            [t2, r2] = meshgrid(wrapTo2Pi(thetaBins3d+pi/2), distanceBins2(1:end-1));
            [x, y] = pol2cart(t2,r2);
...
%             end
            ...
            xs = metric2.*cos(degBins'); % average
            ys = metric2.*sin(degBins');
            
            coordlims=axis;
            
            ang_hd2 = atan2(mean(ys),mean(xs)); % mean direction
            
            mr2 = (cos(ang_hd2).*sum(xs) + sin(ang_hd2).*sum(ys)) / sum(metric2); % mean resultant length
