function [out]= EBCShufflingContinuousParallel(ms,HD,tracking, frameMap, pixX, binarize,elim)
...
FOVsize = 30;
name = 'Shuffling_CircParallel.mat';
...
MRLsave = zeros(length(firing(1,:)),100);                                 %saves all values for all MRL values
...
parfor itteration = 1 : 100                                                    %Loop the number of times you wish to shuffle
...
%         fire(fire < thresh) = 0;                                            %apply threshold on firing
        ...
        if(~isempty(ifire))
            ...
            distanceBinspar = distanceBinspar(1:end-1);                           %Get rid of Infinity
            if any(nspk(:)>0)
                processed = true;
...
savefig('Shuffled_MRL_Hist_Circ.fig')
end
