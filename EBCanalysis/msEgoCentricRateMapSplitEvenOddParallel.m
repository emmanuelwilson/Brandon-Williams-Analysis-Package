function [out, detailed]= msEgoCentricRateMapSplitEvenOddParallel(ms,HD,tracking, frameMap, pixX, pixY, binarize, varargin)
...
FOVsize = 30;
name = 'EBCresultsEvenOddSplitParallelNEW';
%% Setup & Parse
p = inputParser;
...
% ms.HDfiring = ind_fire;                              %Indices of neuron activity for head direction
...
if length(frameMap)> length(ms.FiltTraces(:,1))
    ...
    frameMap = test;
    fprintf('FrameMap is larger than Physiology')    
end
    ...
    if(~isempty(ifire) && ~isempty(ifire1) && ~isempty(ifire2))
        for j = 1 : length(ifire)
            ...
            if j < length(ifire1)
                ...
                cell_time1(j) = mspar.timestamp(ifire1(j));       %Physiological time of firing
            ...
            if j < length(ifire2)
                ...
                cell_time2(j) = mspar.timestamp(ifire2(j));       %Physiological time of firing
...
%             title('nspk')
            ...
            subplot(n,m,c);c=c+1;
            % the +pi/2 brings "forwards" to "up"
            [t2, r2] = meshgrid(wrapTo2Pi(thetaBins+pi/2), distanceBinsPar(1:end-1));
            [x, y] = pol2cart(t2,r2);
...
%             title('nspk 1st half')
            ...
            subplot(n,m,c);c=c+1;
            % the +pi/2 brings "forwards" to "up"
            [t2, r2] = meshgrid(wrapTo2Pi(thetaBins1+pi/2), distanceBinsPar(1:end-1));
            [x, y] = pol2cart(t2,r2);
...
%             title('nspk 2nd half')
            ...
            subplot(n,m,c);c=c+1;
            % the +pi/2 brings "forwards" to "up"
            [t2, r2] = meshgrid(wrapTo2Pi(thetaBins2+pi/2), distanceBinsPar(1:end-1));
            [x, y] = pol2cart(t2,r2);
