function [out, detailed]= msEgoCentricRateMapSplitEvenOddParallel_V2(ms,HD,tracking, frameMap, dimX, dimY, deconvolve, QP)
...
name = 'EBCresultsEvenOddSplitParallelNEWFix';
%% Get behavior information
ratemaps = zeros(FOVsize,120,length(ms.FiltTraces(1,:)));        %Probability ratemap values
ratemaps1 = zeros(FOVsize,120,length(ms.FiltTraces(1,:)));        %Probability ratemap values
ratemaps2 = zeros(FOVsize,120,length(ms.FiltTraces(1,:)));        %Probability ratemap values
...
%         dis2 = dis(round(length(ms.HDfiring(:,1))/2)+1:end,:);
        ...
        thetaBins3d = deg2rad(linspace(-180,180,round(size(dis,2)/3)));                    %angle bins
...
%         distanceBins = distanceBins;
        ...
        occ3d = zeros(120,FOVsize+1);
            occ13d = zeros(120,FOVsize+1);
            occ23d = zeros(120,FOVsize+1);
            
            nspk3d = zeros(120,FOVsize+1);
            nspk13d = zeros(120,FOVsize+1);
            nspk23d = zeros(120,FOVsize+1);
            
            for i = 1 : round(length(thetaBins)/3)
                octemp = sum(occ(i*3-2:i*3,:),1);
                octemp1 = sum(occ1(i*3-2:i*3,:),1);
                octemp2 = sum(occ2(i*3-2:i*3,:),1);
                
                occ3d(i,:) = octemp;
                occ13d(i,:) = octemp1;
                occ23d(i,:) = octemp2;
                
                nspktemp = sum(nspk(i*3-2:i*3,:),1);
                nspktemp1= sum(nspk1(i*3-2:i*3,:),1);
                nspktemp2 = sum(nspk2(i*3-2:i*3,:),1);
...
savefig([pwd,'/',name,'/','Thresh0.1_MRL_Hist.fig'])
end
