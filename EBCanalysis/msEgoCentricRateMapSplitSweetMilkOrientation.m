function [out, detailed]= msEgoCentricRateMapSplitSweetMilkOrientation(ms,HD,tracking, frameMap, pixX, pixY, binarize, varargin)
...
FOVsize = 30;
name = 'EBCresultsSweetMilk_Wall&Object_Split_OrientationChange';
...
ratemapsW = zeros(FOVsize,361,length(ms.FiltTraces(1,:)));        %Probability ratemap values
...
HD = HDmod(QPO,HD(frameMap),tracking(frameMap(:,1),1),tracking(frameMap(:,1),2));
...
for cellNum = 1 : length(ms.FiltTraces(1,:))
    ...
    if(~isempty(ifire) && ~isempty(ifire1) && ~isempty(ifire2))
        for j = 1 : length(ifire)
            ...
            ms.HDfiring(j,cellNum) = HD(ifire(j));%(frameMap(ifire(j)));        %Head Direction of mouse at time of neural firing
            %first half
            if j < length(ifire1)
                ...
                HDfiring1(j,cellNum) = HD(ifire1(j));%(frameMap(ifire1(j)));        %Head Direction of mouse at time of neural firing
            end
            %second half
            if j > length(ifire1)
                tO = j-length(ifire1);
                ind_fire2(tO,cellNum) = ifire2(tO);                      %Add firing index to ms struct
                cell_x2(tO,cellNum) = tracking((frameMap(ifire2(tO))));  %X postion of mouse during firing at sinked time
                cell_y2(tO,cellNum) = tracking((frameMap(ifire2(tO))),2);%Y position of mouse during firing at sinked time
                cell_time2(tO,cellNum) = ms.timestamp(ifire2(tO));       %Physiological time of firing
                HDfiring2(tO,cellNum) = HD(ifire2(tO));%(frameMap(ifire2(tO)));        %Head Direction of mouse at time of neural firing
            end
        end     
...
%             %EBC Metric
            ...
            if binarize == 1
                ...
                colormap(hsv)
                xlim(pixX*[min(tracking(frameMap(ifire1),1)) max(tracking(frameMap(ifire1),1))]);ylim(pixY*[-max(tracking(frameMap(ifire1),2)) -min(tracking(frameMap(ifire1),2))])
...
%             set(gca,'Color','none')
            ...
            if binarize == 1
                ...
                colormap(hsv)
                xlim(pixX*[min(tracking(frameMap(ifire2),1)) max(tracking(frameMap(ifire2),1))]);ylim(pixY*[-max(tracking(frameMap(ifire2),2)) -min(tracking(frameMap(ifire2),2))])
...
%This function will look at the angle of the head in respect to the object
function HDobject = HDmod(QP,HD, rx, ry)
qx = mean(QP(:,1));
qy = mean(QP(:,2));
y3=ry;x3=rx;
y4=ry+100*sin(HD);
...
HDobject = NaN(size(HD));

for i  = 1 : length(HD)
    obj = sqrt((qx - x3(i))^2 +(qy-y3(i))^2);
    heading = sqrt((x3(i) - x4(i))^2+ (y3(i) - y4(i))^2);
    hyp = sqrt((qx - x4(i))^2 +(qy-y4(i))^2);
    HDobject(i) = acos((obj^2+ heading^2-hyp^2)/(2*obj*heading))*180/pi;
end
indNeg = find(HDobject<0);
HDobject(indNeg) = HDobject(indNeg) +360;
end
