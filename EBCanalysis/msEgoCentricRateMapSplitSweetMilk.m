function [out, detailed]= msEgoCentricRateMapSplitSweetMilk(ms,HD,tracking, frameMap, pixX, pixY, binarize, varargin)
...
FOVsize = 30;
name = 'EBCresultsSweetMilk_Split';
%% Setup & Parse
p = inputParser;
...
for cellNum = 1 : length(ms.FiltTraces(1,:))
    ...
    if(~isempty(ifire) && ~isempty(ifire1) && ~isempty(ifire2))
        for j = 1 : length(ifire)
            ...
            if j > length(ifire1)
                t = j-length(ifire1);
                ind_fire2(t,cellNum) = ifire2(t);                      %Add firing index to ms struct
                cell_x2(t,cellNum) = tracking((frameMap(ifire2(t))));  %X postion of mouse during firing at sinked time
                cell_y2(t,cellNum) = tracking((frameMap(ifire2(t))),2);%Y position of mouse during firing at sinked time
                cell_time2(t,cellNum) = ms.timestamp(ifire2(t));       %Physiological time of firing
                HDfiring2(t,cellNum) = HD(frameMap(ifire2(t)));        %Head Direction of mouse at time of neural firing
            end
        end     
        dis1 = dis(1:length(fire1),:);
        dis2 = dis(length(fire1)+1:end,:);
...
%             set(gca, 'YDir','Normal','CLim',[0 prctile(rm(:), 99)])
            ...
            if binarize == 1
                ...
                set(gca,'YDir','Normal')
                caxis([0 360])
%                 axis([min(-pixY*tracking(:,2)) max(-pixY*tracking(:,2)) min(pixX*tracking(:,1)) max(pixX*tracking(:,1))])
                title('Traj')
                axis off
                axis square
            else
...
%             %EBC Metric
            ...
            hold on;
            polarplot([ang_hd2 ang_hd2 ],[0 mr], 'r')
            pol = gca;
            pol.ThetaZeroLocation = 'top';
            pol.ThetaColor = 'none';
            pol.RTickLabel = [];
...
%             set(gca,'Color','none')
            title('Wall Directionality')
            stat = ['MRL: ' num2str(mr) 'Angle : ' num2str(rad2deg(ang_hd2))];
            text(0.2,coordlims(4),stat);
