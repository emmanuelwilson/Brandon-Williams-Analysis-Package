...
hSmall_1 = fspecial('average', 3);
hLarge_1 = fspecial('average', 60);
...
window.size =  40; %The window size to run pixel correlations on was 20
time.size = 20; %The size of the window of bright spot timings to use was 20
seg.initialSize = 2; %initial size of pixel box to include in pixel group G_px was 2
%--------------------------------------------------------------------------
...
for frameNum=1:downSamp:vidObj.numFrames
    ...
    data(:,:,count) = double(vidObj.mask).*msReadFrame(vidObj,frameNum,true,true,false); %loads dF/F frame
    %------------------------ Zaki ----------------------------------------
    data_1(:,:,count) = filter2(hSmall, data(:,:,count));
    data_2(:,:,count) = filter2(hLarge, data(:,:,count));
    data(:,:,count) = data_1(:,:,count) - data_2(:,:,count);
  %  data(:,:,count) = imerode(imadjust(data_3(:,:,count), [0 0.4], [0 1]), se3);
    %----------------------------------------------------------------------
end
frameMax = squeeze(max(data,[],3));
vidObj.frameMax = frameMax;
...
brightSpotsStopAmount = 1; %Counts few than 5 are generally due to noise and slow down the segmentation algorithm
while (sum(brightSpots(:)) > 0) && (numCounts>=brightSpotsStopAmount)
    ...
    corrCoefThresh = corrCoefThreshDefault; %0.9
