function ms = msGenerateVideoObj_EmmALL(dirName, filePrefix)
...
%     folder = dir(folder);
    ...
    for i = 1:length(folder)
        if ~folder(i).isdir && ~isempty(strfind(folder(i).name,filePrefix))
            o(i) = str2num(folder(i).name(6:end-4));
        elseif ~folder(i).isdir && ~isempty(strfind(folder(i).name,'timestamp'))
            stamps(i) = str2num(folder(i).name(10:end-4));
        end
    end
    [a b] = sort(o);
    folder = folder(b);
    ...
    for i=1:length(datFiles)
        if strfind(datFiles(i).name,'timestamp')
            ...
            if i == 1
                ...
                sysClock = [sysClock; max(sysClock(3:end))+sysClocktemp];
