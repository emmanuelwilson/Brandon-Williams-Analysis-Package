...
function [out] = TouchScreenShuffleSeperation(SepDelay,SepFront,SepBack,ms,calInfo,events)
...
%         for j = length(ms.FiltTraces(1,:))
            ...
            if ~isempty(calInfo.Ctrial{c})                
                dshuffledC{c}(:,i) = FullShuffleSplit(CutTraces(shuffledtraces,permute(SepDelay.CorrectTrial{c},[3 2 1]),dc,dFrames*ones(length(dc),1),1),dFrames,1);%Changed +60 to +dFrames                
                fshuffledC{c}(:,i) = FullShuffleSplit(CutTraces(shuffledtraces,permute(SepFront.CorrectTrial{c},[3 2 1]),ctInd,ctCount,2),ctCount,2);
                bshuffledC{c}(:,i) = FullShuffleSplit(CutTraces(shuffledtraces,permute(SepBack.CorrectTrial{c},[3 2 1]),ctInd,ctCount,3),ctCount,3);%Changed +60 to +dFrames
            else
                dshuffledC{c} = [];
                fshuffledC{c} = [];                
                bshuffledC{c} = [];
            ...
            if ~isempty(calInfo.Itrial{c})                
                dshuffledI{c}(:,i) = FullShuffleSplit(CutTraces(shuffledtraces,permute(SepDelay.IncorrectTrial{c},[3 2 1]),di,dFrames*ones(length(di),1),1),dFrames,1);                
                fshuffledI{c}(:,i) = FullShuffleSplit(CutTraces(shuffledtraces,permute(SepFront.IncorrectTrial{c},[3 2 1]),itInd,itCount,2),itCount,2);
                bshuffledI{c}(:,i) = FullShuffleSplit(CutTraces(shuffledtraces,permute(SepBack.IncorrectTrial{c},[3 2 1]),itInd,itCount,3),itCount,3);
            else
                dshuffledI{c} = [];                
                fshuffledI{c} = [];                
                bshuffledI{c} = [];
            ...
            if ~isempty(calInfo.CCtrial{c})         
                dshuffledCcor{c}(:,i) = FullShuffleSplit(CutTraces(shuffledtraces,permute(SepDelay.CorrectCorrectionTrial{c},[3 2 1]),dcc,dFrames*ones(length(dcc),1),1),dFrames,1);                                
                bshuffledCcor{c}(:,i) = FullShuffleSplit(CutTraces(shuffledtraces,permute(SepFront.CorrectCorrectionTrial{c},[3 2 1]),crcInd,crCountC,3),crCountC,3);
                fshuffledCcor{c}(:,i) = FullShuffleSplit(CutTraces(shuffledtraces,permute(SepBack.CorrectCorrectionTrial{c},[3 2 1]),crcInd,crCountC,2),crCountC,2);
            else
                dshuffledCcor{c} = [];
                fshuffledCcor{c} = [];
                bshuffledCcor{c} = [];
            ...
            if ~isempty(calInfo.ICtrial{c})
                dshuffledIcor{c}(:,i) = FullShuffleSplit(CutTraces(shuffledtraces,permute(SepDelay.IncorrectCorrectionTrial{c},[3 2 1]),dic,dFrames*ones(length(dic),1),1),dFrames,1);
                fshuffledIcor{c}(:,i) = FullShuffleSplit(CutTraces(shuffledtraces,permute(SepFront.IncorrectCorrectionTrial{c},[3 2 1]),criInd,crCountI,2),crCountI,2);
                bshuffledIcor{c}(:,i) = FullShuffleSplit(CutTraces(shuffledtraces,permute(SepBack.IncorrectCorrectionTrial{c},[3 2 1]),criInd,crCountI,3),crCountI,3);
            else
                fshuffledIcor{c} = [];
                dshuffledIcor{c} = [];
                bshuffledIcor{c} = [];
...
for c = 1 : NumberOfChoices
    %Correct
    if ~isempty(calInfo.Ctrial{c})
        out.DelaySplithalfcorrect(:,c) = singleSplitShuffle(permute(SepDelay.CorrectTrial{c},[3 2 1]),0,1);
        out.FrontSplithalfcorrect(:,c) = singleSplitShuffle(permute(SepFront.CorrectTrial{c},[3 2 1]),ctCount,2);
        out.BackSplithalfcorrect(:,c) = singleSplitShuffle(permute(SepBack.CorrectTrial{c},[3 2 1]),ctCount,3);
    ...
    %Incorrect
    if ~isempty(calInfo.Itrial{c})
        out.DelaySplithalfincorrect(:,c) = singleSplitShuffle(permute(SepDelay.IncorrectTrial{c},[3 2 1]),0,1);
        out.FrontSplithalfincorrect(:,c) = singleSplitShuffle(permute(SepFront.IncorrectTrial{c},[3 2 1]),itCount,2);
        out.BackSplithalfincorrect(:,c) = singleSplitShuffle(permute(SepBack.IncorrectTrial{c},[3 2 1]),itCount,3);
    ...
    %Correct Correction
    if ~isempty(calInfo.CCtrial{c})
        out.DelaySplithalfccor(:,c) = singleSplitShuffle(permute(SepDelay.CorrectCorrectionTrial{c},[3 2 1]),0,1);
        out.FrontSplithalfccor(:,c) = singleSplitShuffle(permute(SepFront.CorrectCorrectionTrial{c},[3 2 1]),crCountC,2);
        out.BackSplithalfccor(:,c) = singleSplitShuffle(permute(SepBack.CorrectCorrectionTrial{c},[3 2 1]),crCountC,3);
    ...
    %Incorrect Correction
    if ~isempty(calInfo.ICtrial{c})
        out.DelaySplithalficor(:,c) = singleSplitShuffle(permute(SepDelay.IncorrectCorrectionTrial{c},[3 2 1]),0,1);
        out.FrontSplithalficor(:,c) = singleSplitShuffle(permute(SepFront.IncorrectCorrectionTrial{c},[3 2 1]),crCountI,2);
        out.BackSplithalficor(:,c) = singleSplitShuffle(permute(SepBack.IncorrectCorrectionTrial{c},[3 2 1]),crCountI,3);
...
elseif cut == 3 || cut == 2
    for i =1: length(trial(1,1,:))
        trial(:,:,i) = traces(:,ind(i):ind(i)+min(count)-1);
    end
end
out = trial;
end