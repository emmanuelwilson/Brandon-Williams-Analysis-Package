...
function [out, info, rawSep,rawCal] = TouchRaster_V3(events, ms, FA, BA, D, singleCells, sep, savefigures)
...
if D    
    ...
    if sep
        [SepDelay,Ctrial,Itrial,CCtrial,ICtrial] = ChoiceSep_V2(events, correct, incorrect, ctInd,itInd,crcInd,criInd,dctrace,ditrace,dcctrace,dicctrace,ctCount,itCount,crCountC,crCountI,minmax, 1,dirDelay, savefigures);
    ...
    xticklabels(Frame2SecLabels(length(dcrast(1,:)),30,ticmultiplier));
    ...
    xticklabels(Frame2SecLabels(length(dirast(1,:)),30,ticmultiplier));
    ...
    if ~isempty(crcRow)
        ...
        xticklabels(Frame2SecLabels(length(dccrast(1,:)),30,ticmultiplier));
    ...
    if ~isempty(criRow)
        ...
        xticklabels(Frame2SecLabels(length(dicrast(1,:)),30,ticmultiplier));
...
if FA    
    ...
    if sep
        SepFront = ChoiceSep_V2(events, correct, incorrect, ctInd,itInd,crcInd,criInd,Ctrace,Itrace,CRCtrace,CRItrace,ctCount,itCount,crCountC,crCountI,minmax,2,dirFront,savefigures);
...
%     end
    ...
    xticklabels(Frame2SecLabels(length(Crast(1,:)),30,ticmultiplier));
    ...
    xticklabels(Frame2SecLabels(length(Irast(1,:)),30,ticmultiplier));
    ...
    if ~isempty(crcRow)
        ...
        xticklabels(Frame2SecLabels(length(CRCrast(1,:)),30,ticmultiplier));
    ...
    if ~isempty(criRow)
        ...
        xticklabels(Frame2SecLabels(length(CRIrast(1,:)),30,ticmultiplier));
...
if BA
    ...
    if sep
        SepBack = ChoiceSep_V2(events, correct, incorrect, ctInd,itInd,crcInd,criInd,Ctrace,Itrace,CRCtrace,CRItrace,ctCount,itCount,crCountC,crCountI,minmax,3,dirBack,savefigures);
...
if ~isempty(ct) && length(ct(1,1,:))>1
    ...
    for i = 1 : length(ct(:,1))
        ...
        xticklabels(Frame2SecLabels(length(temp(1,:)),30,multiplier));
