...
function out = MeanFluoAcrossDays_V2(folderpath,cut)
oldpath = pwd;
cd(folderpath)
load('aligmentMaps','Singlemap')
SepFiles = dir(['Reliability/*UncutTraces*.mat']);
bmap = Singlemap;
bmap(bmap > 0) = 1;
...
for anchor = 1 : length(cut)
    ...
    for i = 1 : length(SepFiles)
        if (str2num(SepFiles(i).name(12:end-4)) == i)
            sep =load(['Reliability/UncutTraces',num2str(i)],'rawSep');
            varname = fields(sep);
            sep = sep.(varname{1});
            ttype = fields(sep);
            ...
            for j = 1 : length(ind)
                ...
                if cellnum > 0
                    for c = 1 : ChoiceNum
                        if any(strcmp(subnames, [prefix,'correct'])) && ~isempty(sep.(ttype{cut*4-3}){c})
                            CorrectComp{c}(j,1:length(sep.(ttype{cut*4-3}){c}(1,:,1)),i) = mean(sep.(ttype{cut*4-3}){c}(cellnum,:,:),1);
                            CorrectAD{c}(count1{c} : count1{c} + length(sep.(ttype{cut*4-3}){c})-1,1:length(sep.(ttype{cut*4-3}{c})),j) = sep.(ttype{cut*4-3}){c}(:,:,cellnum);
                            count1{c} = count1{c} + length(sep.(ttype{cut*4-3}){c}(:,1,1));
                            if j ==1
                                behaveP(1,i) = behaveP(1,i) + length(sep.(ttype{cut*4-3}){c}(:,1,1));
                            end
                        end
                        if any(strcmp(subnames, [prefix,'IncorrectTrial'])) && ~isempty(sep.(ttype{cut*4-2}){c})
                            IncorrectComp{c}(j,1:length(sep.(ttype{cut*4-2}){c}(1,:,1)),i) = mean(sep.(ttype{cut*4-2}){c}(:,:,cellnum),1);
                            IncorrectAD{c}(count2{c} : count2{c} + length(sep.(ttype{cut*4-2}){c}(:,1,1))-1,1:length(sep.(ttype{cut*4-2}){c}(1,:,1)),j) = sep.(ttype{cut*4-2}){c}(:,:,cellnum);
                            count2{c} = count2{c} + length(sep.(ttype{cut*4-2}){c}(:,1,1));
                            if j == 1
                                behaveP(2,i) = behaveP(2,i) + length(sep.(ttype{cut*4-2}){c}(:,1,1));
                            end
                        end
                        if any(strcmp(subnames, [prefix,'CorrectCorrectionTrial'])) && ~isempty(sep.(ttype{cut*4-1}){c})
                            CorrectCorComp{c}(j,1:length(sep.(ttype{cut*4-1}){c}(1,:,1)),i) = mean(sep.(ttype{cut*4-1}){c}(:,:,cellnum),1);
                            CorrectCorrectionAD{c}(count3{c} : count3{c} + length(sep.(ttype{cut*4-1}){c}(:,1,1))-1,1:length(sep.(ttype{cut*4-1}){c}(1,:,1)),j) = sep.(ttype{cut*4-1}){c}(:,:,cellnum);
                            count3{c} = count3{c} + length(sep.(ttype{cut*4-1}){c}(:,1,1));
                            if j == 1
                                behaveP(3,i) = behaveP(3,i) + length(sep.(ttype{cut*4-1}){c}(:,1,1));
                            end
                        end
                        if any(strcmp(subnames, [prefix,'IncorrectCorrectionTrial'])) && ~isempty(sep.(ttype{cut*4}){c})
                            IncorrectCorComp{c}(j,1:length(sep.(ttype{cut*4}){c}(1,:,1)),i) = mean(sep.(ttype{cut*4}){c}(:,:,cellnum),1);
                            IncorrectCorrectionAD{c}(count4{c} : count4{c} + length(sep.(ttype{cut*4}){c}(:,1,1))-1,1:length(sep.(ttype{cut*4}){c}(1,:,1)),j) = sep.(ttype{cut*4}){c}(:,:,cellnum);
                            count4{c} = count4{c} + length(sep.(ttype{cut*4}){c}(:,1,1));
                            if j == 1
                                behaveP(4,i) = behaveP(4,i) + length(sep.(ttype{cut*4}){c}(:,1,1));
                            end
                        end
                    end
                end
