...
function [out, info] = TouchRaster(events, ms, FA, BA,D, singleCells,sep, splitd, savefigures)
dirDelay = 'Delay';
...
if D
    ...
    if singleCells
        IndividualDelay = IndividualCells(dctrace,ditrace,dcctrace,dicctrace,ctCount,itCount,crCountC,crCountI,minmax, 1,dirDelay, savefigures);
    end
    if splitd
    ...
    if sep
        [SepDelay,Ctrial,Itrial,CCtrial,ICtrial] = ChoiceSep(events, correct, incorrect, ctInd,itInd,crcInd,criInd,dctrace,ditrace,dcctrace,dicctrace,ctCount,itCount,crCountC,crCountI,minmax, 1,dirDelay, savefigures);
...
if FA
    if singleCells
        IndividualFront = IndividualCells(Ctrace,Itrace,CRCtrace,CRItrace,ctCount,itCount,crCountC,crCountI,minmax,2,dirFront,savefigures);
    end
    
    if sep
        SepFront = ChoiceSep(events, correct, incorrect, ctInd,itInd,crcInd,criInd,Ctrace,Itrace,CRCtrace,CRItrace,ctCount,itCount,crCountC,crCountI,minmax,2,dirFront,savefigures);
...
if BA
    ...
    if singleCells
        IndividualBack = IndividualCells(Ctrace,Itrace,CRCtrace,CRItrace,ctCount,itCount,crCountC,crCountI,minmax,3, dirBack,savefigures);
    end
    
    if sep
        SepBack = ChoiceSep(events, correct, incorrect, ctInd,itInd,crcInd,criInd,Ctrace,Itrace,CRCtrace,CRItrace,ctCount,itCount,crCountC,crCountI,minmax,3,dirBack,savefigures);
...
function out = IndividualCells(ct,it,cct,ict,cl,il,ccl,icl,minmax,cut,dirName,save)
figure(1)
n = 5;
m = 4;
minlim = minmax(:,1);
