...
function [out,ccell,icell,cccell,iccell] = ChoiceSep_V3_invisible(events, cchoice, ichoice, cind,iind,ccind,icind,ct,it,cct,ict,cl,il,ccl,icl,minmax,cut,dirName,save)
...
for posnum = 1 : length(totpoke)
    ...
    if ~isempty(ct) &&  ~isempty(ccell{poke}) %%length(ct(1,1,:))>1 &&
        ...
        for i = 1 : length(ct(:,1))
            ...
            xticklabels(Frame2SecLabels(length(temp(1,:)),30,ticmultiplier));
    ...
    if ~isempty(raster)
        xticks(1:fps:length(raster(1,:)));
        xticklabels(Frame2SecLabels(length(raster(1,:)),30,ticmultiplier));
        yticks(1:round(length(raster(:,1))/10):length(raster(:,1)));
    end
    xlabel('Time(Seconds)')
    ylabel('Cell ID')
    %Incorrect
