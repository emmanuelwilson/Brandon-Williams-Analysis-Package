function synchronization_gui_V3()
...
% point to the folder containing the data
    ...
    function f_proceed(source,event)
        ...
        if subfolder==1
            ...
            resp = exist([my_path '\timestamp.dat'],'file');
            if resp==0
                waitfor(msgbox(['Cannot find file ' my_path '\timestamp.dat. Stopping here...']));
            else
                dataset_ok = dataset_ok + 1;
            end
            
            % check if schedules.csv file exists
            resp = ~isempty(dir([my_path '\*.csv']));
            if resp==0
                waitfor(msgbox(['Cannot find Touchscreen CSV file in:' my_path '. Stopping here...']));
            else
                dataset_ok = dataset_ok + 1;
            end
            ...
            if dataset_ok==4
                single_folder_synchronization_V2(my_path);
                close all
            end
        else
